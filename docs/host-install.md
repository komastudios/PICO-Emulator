...
- **Service user** `android`, plus the `sitegroup` group. Existing accounts are left untouched — group membership is added, the account definition is not rewritten. The user needs `kvm` for acceleration and `sitegroup` for package access.
